function [F,J] = calibration_bundle(x, I, lambda, theta_a, alpha_g, alpha_a)
                ...
                for l=1:nb_alpha_a
                    ...
                    if (nargout > 1)
                        ...
                        if ~isnan(I(ind))
                            ...
                            J(ind,1) = b(i)*A*dG_dalpha0g;
                            J(ind,2) = b(i)*A*dG_dk1g;
                            J(ind,3) = b(i)*A*dG_dk2g;
                            J(ind,4) = b(i)*dA_dalpha0a*G;
                            J(ind,5) = b(i)*dA_dk1a*G;
                            J(ind,6) = b(i)*dA_dk2a*G;
                            J(ind,7) = b(i)*dA_dtheta0a*G;
                            J(ind,8:end) = 0;
                            J(ind,7+i) = A*G;
                        end
                    end
